ui <- shiny::fluidPage(
  ...
  shinyjs::useShinyjs(),
  shiny::titlePanel("cleanRanalyzR: Generate cleaning log, clean dataset and run Analysis"),
  shiny::sidebarLayout(
    shiny::sidebarPanel(
      shiny::selectizeInput(
        'operation', "What action do you want to perform ?", choices = setNames(c("clog","clean","makedap","analysis","xml_tolabel","label_toxml"),c("Produce a cleaning LOG","Clean DATA","Make Analysis Plan","Run Analysis","Convert XML to LABEL","Convert LABEL to XML")),
        options = list(
    ...
    shiny::mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  tabPanel("SOPs",
      p("Welcome to cleanRanalyzR shiny app. This tool was developed to make the typical workflow of AO's, Data/Database Officers smoother and less complicated. The app will help users run their data checks, clean the data and perform analysis in a more efficient and automated way. No need for strong R background and technical capacities."),
      p("What this app will do for you :"),
      p("1/ Produce a cleaning LOG"),
      p("2/ Clean DATA"),
      p("3/ Make Analysis Plan"),
      p("4/ Run Analysis"),
      p("5/ Convert XML to LABEL"),
      p("6/ Convert LABEL to XML"),
      ...
      p("Users can upload data either in a .csv or .xlsx/.xls extension",code("Data format is really IMPORTANT to be able to use this app without facing any issues.")),
      ...
      p("Upload your XLSform (data collection tool) with the", strong("survey")," and", strong("choices"), "tabs. Place your questions of type text for", strong("other/autre"), "immediately after their parent questions (select_one/select_multiple). To detect all 'other' columns, headers should end with other/autre."),
      ...
      downloadButton("full_logdef", "Download log_definition example"),
      ...
      p(span("parent.other.question",style ="color:#EE5859"), ": The cleaning log will list all 'other/autre' answers, this column will have the parent xml question."),
      p("Example: question.name: food_source_other, parent.other.question: food_source"),
      ...
      p("Example: question.name: food_source_other, parent.other.question: food_source, parent.other.answer: own_production food_aid other"),
      ...
      downloadButton("full_log","Download cleaningLOG example"),
      tableOutput("ex_log"),
      h3(strong("Make Analysis Plan"),style ="color:#EE5859"),
      p("To have a generic Data Analysis Plan, Upload your questionnaire workbook and click on ",strong("Make DAP")," button."),
      p("Once you download your DAP, review it to change Hypothesis, Add or remove analysis cases."),
      ...
      p(strong("REMINDER: group separator should be a point (.) and not a slash (/), do not include groups in headers, do not have duplicated columns. If questions/options were added, update your XLSform tool. KOBOtoolbox will warn you when you are trying to upload a tool that has a duplicated 'name' which is not the case if you have duplicated 'label'. Be careful, you should have unique labeled headers to run this option.")),
      p("Once users uploaded the required inputs, the button", strong("CONVERT data"), "will be enabled. Once computations are done, the button", strong("Download Converted DATA"), "will be also enabled to download the Output."),
      shiny::br()
      ),
    tabPanel("logs",pre(id = "console"))
  )
)
)
)