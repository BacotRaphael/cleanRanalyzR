...
from_xml_tolabel<-function(db,choices,survey,choice_labels,survey_labels){
  multiple_choices <- filter(survey, str_detect(type, "(\\bselect_multiple\\b)"))$name
  ...
  # names(survey)<-gsub(":.*","",names(survey))
  choice_labels <- choices[[choice_labels]]
  survey_labels <- survey[[survey_labels]]
...
from_label_toxml<-function(db,choices,survey,choices_labels,survey_labels){
  # names(choices)<-gsub(":.*","",names(choices))
  # names(survey)<-gsub(":.*","",names(survey))
  multiple_choices <- filter(survey, str_detect(type, "(\\bselect_multiple\\b)"))[[survey_labels]]
  chr_names<-db %>% select_if(~ !(all(is.na(.x)))) %>% select_if(~ is.character(.)) %>% names
  choice_xml <- choices[["name"]]
  survey_xml <- survey[["name"]]
  for (i in 1:length(chr_names)){
    if(chr_names[i]%!in%multiple_choices){
      var_xml <- match(db[[chr_names[i]]], choices[[choices_labels]])
  ...
  for (i in 1:length(names(db))){
    if(names(db)[i]%in%(multiple_choices)){
      ...
      names(db)[which(names(db)%in%choices_sm)]<-lapply(split_sm, function(x)match(x, choices[[choices_labels]])) %>% 
        lapply(.,function(x){ifelse(is.na(x),x,choice_xml[x])}) %>% lapply(., function(x)paste(x,collapse = ".")) %>% unlist %>%
      sub("NA",survey_xml[match(names(db)[i], survey[[survey_labels]])],.)
      names(db)[i]<-survey_xml[match(names(db)[i], survey[[survey_labels]])]
    } else{
      var_label <- match(names(db)[i], survey[[survey_labels]])
